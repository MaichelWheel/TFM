...
# ---------------------------------------------------- #
# This is a MIT-BIH database ECG signal PCA (Class V)
# ---------------------------------------------------- #
...
#for i in range (1): # DEBUG: To avoid reading every file
    ...
    if filename.endswith(".dat") and not filename.startswith(('102','104','107','217')):
        ...
        for it, beat in enumerate(ann.symbol):
            if beat == 'V':# in good_beats: # ERROR: ONLY 2 BEATS
...
app.layout = html.Div(children=[
    html.H1(children='Graph ECG with Dash: Beats V'),
...
def update_graph(value, value2, value3, value4, value5, value6, value7):
    p1 = 5*value
    p2 = 5*value2
    p3 = 5*value3
    p4 = 5*value4
    ...
    Result_Vector = mv + p1*pc[0] + p2*pc[1] + p3*pc[2] + p4*pc[3] + p5*pc[4] + p6*pc[5] + p7*pc[6]
